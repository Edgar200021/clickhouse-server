	...
	describe("Get Product Sku", () => {
		it("Should return 200 status code when request is successfull", async () => {
			const getProductSkuResponse = await testApp.withSignIn<
				Parameters<typeof testApp.getProductSku>["1"][],
				WithSignIn<Parameters<typeof testApp.getProductSku>["1"][]>
			>(
				{ body: user },
				{
					fn: testApp.getProductSku,
		...
		it("Should return 400 status code when data is invalid", async () => {
			...
			>(
				...
				testCases.map((t) => ({
					fn: testApp.getProductSku,
					additionalArg: [t] as Parameters<typeof testApp.getProductSku>["1"][],
				})),
				UserRole.Admin,
			);
